...
<ul class="nav nav-tabs">
	<li <?php  if($operation == 'post') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createWebUrl('index2', array('op' => 'post'))?>">添加商品</a></li>
	<li <?php  if($operation == 'display') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createWebUrl('index2', array('op' => 'display'))?>">管理商品</a></li>
</ul>
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<div class="tab-content">
					<div class="tab-pane  active" id="tab_basic"><?php (!empty($this) && $this instanceof WeModuleSite || 1) ? (include $this->template('index2_basic', TEMPLATE_INCLUDEPATH)) : (include template('index2_basic', TEMPLATE_INCLUDEPATH));?></div>
					<div class="tab-pane" id="tab_des"><?php (!empty($this) && $this instanceof WeModuleSite || 1) ? (include $this->template('index2_des', TEMPLATE_INCLUDEPATH)) : (include template('index2_des', TEMPLATE_INCLUDEPATH));?></div>
...
<div class="main">
	...
	<div class="panel-body">
		<form action="./index.php" method="get" class="form-horizontal" role="form">
			...
			<input type="hidden" name="m" value="shoping11" />
			<input type="hidden" name="do" value="index2" />
			...
			<div class="form-group">
				...
				<div class="col-sm-8 col-xs-12">
					<?php  echo tpl_form_field_category_2level('index2', $parent, $children, $params[':pcate'], $params[':ccate'])?>
				</div>
		...
		<table class="table table-hover">
			<thead class="navbar-inner">
				<tr>
					...
					<th style="width:9%;">商品规格-1</th>
					<th style="width:6%;">库存-1</th>
					<th style="width:9%;">商品规格-2</th>
                                                                          <th style="width:6%;">库存-2</th>
					<th style="width:9%;">商品规格-3</th>
                                                                          <th style="width:6%;">库存-3</th>
					<th style="width:9%;">商品规格-4</th>
                                                                           <th style="width:6%;">库存-4</th>
					<th style="width:9%;">商品规格-5</th>
                                                                            <th style="width:6%;">库存-5</th>
...
<!--						<label data='<?php  echo $item['ishot'];?>' class='label label-default <?php  if($item['ishot']==1) { ?>label-info<?php  } ?>' onclick="setProperty(this,<?php  echo $item['id'];?>,'hot')">热卖</label>
					...
					<td><?php  echo $item['size1'];?></td>
					<td><?php  echo $item['total1'];?></td>
					<td><?php  echo $item['size2'];?></td>
					<td><?php  echo $item['total2'];?></td>
					<td><?php  echo $item['size3'];?></td>
					<td><?php  echo $item['total3'];?></td>
					<td><?php  echo $item['size4'];?></td>
					<td><?php  echo $item['total4'];?></td>
					...
					<td style="text-align:right;">
						<a href="<?php  echo $this->createWebUrl('index2', array('id' => $item['id'], 'op' => 'post'))?>"class="btn btn-default btn-sm" data-toggle="tooltip" data-placement="top" title="编辑"><i class="fa fa-pencil"></i></a>&nbsp;&nbsp;
						<a href="<?php  echo $this->createWebUrl('index2', array('id' => $item['id'], 'op' => 'delete'))?>" onclick="return confirm('此操作不可恢复，确认删除？');return false;" class="btn btn-default btn-sm" data-toggle="tooltip" data-placement="top" title="删除"><i class="fa fa-times"></i></a>
					</td>
