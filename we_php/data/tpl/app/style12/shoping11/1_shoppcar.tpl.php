...
<div class="head">
                <img src="../addons/shoping11/style/img/wei1.png" style ="height: 2em;">
...
<div class="shopcart-main img-rounded">
	<div style='text-align:center;padding:50px 0 50px 0; <?php  if(count($list)>0) { ?>display:none<?php  } ?>' id='cartempty'>
		<img src='../addons/shoping11/style/img/icon_cart_empty.png' /><br/><br/>
	...
	<div class="shopcart-item clearfix" id='item_<?php  echo $item['id'];?>' style='height:<?php  if(!empty($goods['optionname'])) { ?>140px;<?php  } else { ?>120px<?php  } ?>'>
		<img src="<?php  echo tomedia($goods['thumb']);?>" style="width: 4em;height: 4em;">
			...
			<div class="clearfix">
				<div class="input-group pull-left">
					<span class="input-group-btn">
                                            <button class="btn btn-default btn-sm" type="button" onclick="reduceNum(<?php  echo $item['id'];?>)"><i class="fa fa-minus"><img src="../addons/shoping11/style/img/delete.png" style="width: 1em;height: 1em;"></i></button>
					</span>
					...
					<span class="input-group-btn">
						<button class="btn btn-default btn-sm" type="button" onclick="addNum(<?php  echo $item['id'];?>,<?php  if($goods['max'] == 0) { ?><?php  echo $goods['total'];?><?php  } else { ?><?php  echo min($goods['total'], $goods['maxbuy'])?><?php  } ?>)"><i class="fa fa-plus"><img src="../addons/shoping11/style/img/add.png" style="width: 1em;height: 1em;"></i></button>
					</span>
				</div>
...
<script type="text/javascript">
	...
	function clearCart(){
		if (confirm('确定要清空购物车吗？')) {
			tip("正在处理数据...");
			$.getJSON('<?php  echo $this->createMobileUrl('shoppcar',array('op'=>'clear'));?>', function(s){
	...
	function removeCart(id){
		if (confirm('您确定要删除此商品吗？')) {
			tip("正在处理数据...");
			var url = "<?php  echo murl('entry//shoppcar',array('m'=>'shopping11','op'=>'remove'), true)?>"+ "&id=" + id;
	...
	function updateCart(id,num){
		var url = "<?php  echo murl('entry//shoppcar',array('m'=>'shopping11','op'=>'update'), true)?>"+ "&id=" + id+"&num=" + num;
		$.getJSON(url, function(s){
